...
echo "Showing results between start " . $start . " and " . $end ." UTC <br>";
...
$result = mysqli_query($con,"select DATE_FORMAT(DATETIME, '%Y%m%d %H%i'), RX_FREQ, RX_ID, MIN(RAW_FROM_MMSI), FROM_NAME, FROM_CTRY, ECC from newlog where (datetime>date_sub(utc_date(), interval $p day) and datetime<date_sub(utc_date(), interval $pp day)) and (hour(datetime) >= $start and hour(datetime) < $end ) and rx_freq like '%".$rx_freq."%' and FROM_TYPE='COAST' and RAW_FROM_MMSI not like '%~%' and FROM_NAME not like '%".$unid."%' and ECC like '%".$eccc."%' and RX_ID like '%".$rx."%'  group by RAW_FROM_MMSI order by RAW_FROM_MMSI"); 

//$result = mysqli_query($con,"select DATE_FORMAT(DATETIME, '%Y%m%d %H%i'), RX_FREQ, RX_ID, MIN(RAW_FROM_MMSI), FROM_NAME, FROM_CTRY, ECC from newlog where (datetime>date_sub(utc_date(), interval $p day) and datetime<date_sub(utc_date(), interval $pp day))  and rx_freq like '%".$rx_freq."%' and FROM_TYPE='COAST' and RAW_FROM_MMSI not like '%~%' and FROM_NAME not like '%".$unid."%' and ECC like '%".$eccc."%' and RX_ID like '%".$rx."%'  group by RAW_FROM_MMSI order by RAW_FROM_MMSI"); 
