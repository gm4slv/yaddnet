...
$result = mysqli_query($con,"SELECT * FROM newlog where datetime > date_sub(utc_timestamp(), interval 24 hour) and (( FROM_TYPE ='SHIP' and left(FROM_MID, 1) = '".$mid."') or (TO_TYPE = 'SHIP' and left(TO_MID, 1) = '".$mid."')) and ECC='OK' and RAW_FROM_MMSI not like '%~%' and RAW_TO_MMSI not like '%~%' and EOS not like '%~%' $notest_string $urgdis_string order by datetime desc");
